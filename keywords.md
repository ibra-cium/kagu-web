...
| `.` | — | T_DOT | only before `erthikana()` or `erman()` |
...
Watch for a collision the new keywords introduce: `man` is an alias of `erman`, and `naam` is a common variable name. They are distance 2 apart, but `naam` resolves as an identifier first because alias lookup runs before suggestion, so no suggestion is produced. There is a test case for this.
